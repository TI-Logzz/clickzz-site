...
export function Logo({ height = 28, mark = false, light = false }: { height?: number; mark?: boolean; light?: boolean }) {
  ...
  const src = light ? '/brand/clickzz-logo-white@120.png' : '/brand/clickzz-logo@120.png';
  return (
    <img
      src={src}
      alt="Clickzz"
...
export function LogoMark({ size = 28 }: { size?: number }) {
  return <img src="/brand/clickzz-mark-256.v2.png" alt="" aria-hidden="true" width={size} height={size} style={{ width: size, height: size, display: 'block', borderRadius: size * 0.22 }} decoding="async" />;
}
